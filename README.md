...
#### 3) Initialize Wizard ####
After setting up the wizard's cards, you can initilize it.
...
`backdrop`            | boolean or `'static'` | `true` | Includes a modal-backdrop element. Alternatively, specify `static` for a backdrop which doesn't close the modal on click.
...
`buttons`             | object   |  `{cancelText: 'Cancel', nextText: 'Next', backText: 'Back', submitText: 'Submit', submittingText: 'Submitting...'}` | 
...
`submitError`   | Triggers when submit encounters an error
`loading`       | Triggers after the submit event
`readySubmit`   | Triggers when the wizard has reached its final card
...
When the wizard reaches its last step and the user clicks the submit button, all of the inputs from all of the wizard cards will be aggregated together and `POST`-ed to your `submitUrl`.
